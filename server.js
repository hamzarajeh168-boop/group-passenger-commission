...
function emptyStats() { return { passengers: 0, orders: 0, groupFloor: 0, fullCars: 0, other: 0, totalTrips: 0, totalCommission: 0 }; }
function captainStats(captain) { captain.stats = { ...emptyStats(), ...(captain.stats || {}) }; return captain.stats; }
function categoryFor(rule, text) {
  if (rule && rule.category && ['passengers', 'orders', 'groupFloor', 'other'].includes(rule.category)) return rule.category;
  const value = `${String(rule && rule.label || '')} ${String(text || '')}`.toLocaleLowerCase('ar');
  if (value.includes('أرضية') || value.includes('ارضية') || value.includes('floor')) return 'groupFloor';
...
function ruleFor(text, rules) { const value = String(text || '').toLocaleLowerCase('ar'); return (rules || []).find(r => r.enabled !== false && (r.direction || 'consumption') === 'consumption' && r.match && value.includes(String(r.match).toLocaleLowerCase('ar'))) || null; }
function eventGroupId(event) {
  return String(event.groupId || event.chatId || event.data && event.data.chatId || '').trim();
}
async function removeFromGroup(phone, groupId) {
  if (baileys) {
    ...
    try {
      const response = await fetch(controlUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-baileys-key': process.env.BAILEYS_CONTROL_KEY || 'shahm-local' }, body: JSON.stringify({ groupId, phone }) });
      if (!response.ok) return { attempted: true, performed: false, reason: `Baileys returned HTTP ${response.status}` };
      return { attempted: true, performed: true };
    } catch (error) {
      return { attempted: true, performed: false, reason: `Baileys غير متصل: ${error.message}` };
    }
  }
...
function captainView(captain, ledger) {
  const stats = captainStats(captain);
  const entries = ledger.filter(entry => entry.captainId === captain.id).slice(0, 100);
  return {
    captain: { name: captain.name, code: captain.code, phone: captain.phone, balance: captain.balance, status: captain.status, stats },
    ledger: entries,
    totals: {
      charges: money(entries.filter(entry => entry.type === 'commission').reduce((sum, entry) => sum + Math.abs(Number(entry.amount) || 0), 0)),
      topups: money(entries.filter(entry => entry.type === 'topup').reduce((sum, entry) => sum + Math.max(0, Number(entry.amount) || 0), 0))
    }
  };
}
app.post('/api/captain/login', (req, res) => {
  const value = String(req.body.phoneOrCode || '').trim();
  const normalizedPhone = value.replace(/\D/g, '');
  const s = load();
  const captain = s.captains.find(item => item.code.toLowerCase() === value.toLowerCase() || item.phone === normalizedPhone);
  if (!captain) return res.status(401).json({ error: 'رقم الهاتف أو كود الكابتن غير صحيح' });
  res.json(captainView(captain, s.ledger));
});
app.post('/webhook/group', async (req, res) => { const event = req.body || {}; const s = load(); const configuredGroupId = String(s.settings.groupId || '').trim(); const receivedGroupId = eventGroupId(event); if (!configuredGroupId) return res.status(409).json({ ok: false, status: 'group-not-configured', reason: 'يجب تحديد Group ID من غرفة الإدارة أولًا' }); if (!receivedGroupId || receivedGroupId !== configuredGroupId) return res.json({ ok: true, status: 'ignored-group', groupId: receivedGroupId || null }); if (event.type === 'message' && event.id) { if (!s.messages.some(m => m.id === event.id)) s.messages.push({ id: event.id, groupId: receivedGroupId, from: String(event.from || ''), text: String(event.text || ''), replyTo: String(event.replyTo || ''), createdAt: new Date().toISOString() }); save(s); return res.json({ ok: true, status: 'message-saved' }); } if (event.type !== 'like') return res.json({ ok: true, status: 'ignored' }); if (s.likes.some(x => x.id === event.id)) return res.json({ ok: true, status: 'duplicate' }); const reply = s.messages.find(m => m.id === event.replyId && m.groupId === configuredGroupId); const original = reply && s.messages.find(m => m.id === reply.replyTo && m.groupId === configuredGroupId); const captain = s.captains.find(c => c.phone === String(event.from || '').replace(/\D/g, '')); const rule = ruleFor(original && original.text, s.settings.rules); const record = { id: event.id || id(), groupId: receivedGroupId, captainPhone: event.from, replyId: event.replyId, requestText: original ? original.text : '', status: 'rejected', createdAt: new Date().toISOString() }; let insufficientBalance = false; if (!captain || !reply || !original || event.emoji !== s.settings.likeEmoji || !rule) record.reason = 'لا يوجد سياق مكتمل أو قاعدة استهلاك مطابقة'; else if (captain.status !== 'active') record.reason = 'حساب الكابتن موقوف أو محظور'; else if (captain.balance < MINIMUM_BALANCE) { insufficientBalance = true; record.reason = `الرصيد أقل من الحد الأدنى ${MINIMUM_BALANCE.toFixed(2)} - يمنع أخذ أي طلب وسيتم إزالة الرقم من الجروب`; } else if (captain.balance < rule.commission) { insufficientBalance = true; record.reason = 'الرصيد أقل من عمولة هذا الطلب - يمنع أخذ الطلب وسيتم إزالة الرقم من الجروب حتى يتم الشحن'; } else { captain.balance = money(captain.balance - rule.commission); const stats = captainStats(captain); const category = categoryFor(rule, original.text); stats[category] += 1; stats.totalTrips += 1; stats.totalCommission = money(stats.totalCommission + rule.commission); record.status = 'charged'; record.commission = rule.commission; record.rule = rule.label; record.category = category; s.ledger.unshift({ id: id(), captainId: captain.id, phone: captain.phone, type: 'commission', amount: -rule.commission, rule: rule.label, category, requestText: original.text, replyId: reply.id, createdAt: record.createdAt }); if (captain.balance < MINIMUM_BALANCE) insufficientBalance = true; } if (insufficientBalance) record.groupRemoval = await removeFromGroup(captain.phone, configuredGroupId); s.likes.unshift(record); save(s); res.json({ ok: true, ...record, minimumBalance: MINIMUM_BALANCE, removeLike: false }); });
